...
def get_vsllm_gwent_draw(
    account_config: "AccountConfig",
) -> Generator[tuple[bool, dict], None, None]:
    """维云 (vsllm.com) 翻牌抽卡

    通过 /api/gwent/status 检查剩余次数，/api/gwent/draw 执行翻卡。
    奖励直接充值到账户，不返回 CDK，因此成功时返回空 code。

    依赖：调用方需在执行前将登录后的 cookies 存入 account_config.extra["_session_cookies"]。

    Args:
        account_config: 账号配置对象，需包含 _session_cookies 在 extra 中

    Yields:
        tuple[bool, dict]: (True, {"code": ""}) 成功（奖励直接到账，无需充值），
                           (False, {"error": "msg"}) 失败
    """
    account_name = account_config.get_display_name()

    # 从 session cookies 获取登录态（由 check_in_with_cookies 在调用前存入）
    session_cookies = account_config.get("_session_cookies")
    if not session_cookies:
        print(f"❌ {account_name}: _session_cookies not found, cannot perform gwent draw")
    ...
    try:
        ...
        try:
            ...
            headers = {
                ...
                "pragma": "no-cache",
                "origin": origin,
                "referer": f"{origin}/console/personal",
                "sec-ch-ua": '"Google Chrome";v="143", "Chromium";v="143", "Not A(Brand";v="24"',
                "sec-ch-ua-mobile": "?0",
            ...
            remaining = 0
            if status_response.status_code == 200:
                status_data = response_resolve(status_response, "get_gwent_status", account_name)
                if status_data and status_data.get("success"):
                    data = status_data.get("data", {})
                    if not data.get("enabled", True):
                        print(f"ℹ️ {account_name}: Gwent draw is disabled")
                        yield True, {"code": ""}
                        return
                    remaining = int(data.get("charges_current", 0))
                    print(f"ℹ️ {account_name}: Gwent charges remaining: {remaining}")
                else:
                    msg = status_data.get("message", "Unknown error") if status_data else "Invalid response"
                    print(f"⚠️ {account_name}: Failed to get gwent status: {msg}")
            else:
                print(f"⚠️ {account_name}: Gwent status HTTP {status_response.status_code}")

            if remaining <= 0:
                print(f"ℹ️ {account_name}: No gwent draws remaining today")
                yield True, {"code": ""}
                return
            ...
            while remaining > 0:
                response = session.post(
                    f"{origin}/api/gwent/draw",
                    headers=headers,
                    timeout=30,
                )

                if response.status_code in [200, 400]:
                    json_data = response_resolve(response, "execute_gwent_draw", account_name)
                    if json_data is None:
                        break
                    ...
                    if json_data.get("success"):
                        ...
                        # 从响应中更新剩余次数
                        remaining = int(data.get("charges_current", remaining - 1))
                        draw_count += 1

                        # 提取并打印奖励信息
                        prize_name = data.get("prize_name", data.get("label", data.get("name", "")))
                        quota = data.get("quota", data.get("quota_amount", data.get("amount", "")))
                        message = data.get("message", json_data.get("message", ""))
                        print(
                            f"✅ {account_name}: Gwent draw #{draw_count} successful! "
                            f"prize={prize_name}, quota={quota}, msg={message}, remaining={remaining}"
                        )
                        # 奖励直接到账，返回空 code 表示不需要充值
                        yield True, {"code": ""}
                        continue

                    # 失败处理
                    message = json_data.get("message", json_data.get("msg", ""))
                    if any(kw in message for kw in ["次数", "冷却", "cooldown", "no more", "已经", "已抽"]):
                        print(f"ℹ️ {account_name}: No more gwent draws available (cooldown or exhausted): {message}")
                        break

                    print(f"❌ {account_name}: Gwent draw #{draw_count + 1} failed - {message}")
                    break
                else:
                    print(f"❌ {account_name}: Gwent draw HTTP {response.status_code}")
                    break

            if draw_count > 0:
                print(f"✅ {account_name}: Total {draw_count} gwent draw(s) completed")

        finally:
            session.close()
    except Exception as e:
        print(f"❌ {account_name}: Error during gwent draw - {e}")
        yield False, {"error": f"Error during gwent draw - {e}"}
